...
def scrape_product_details(product_url, session, output_dir, script_name, brands_list, current_product_number, total_products_found, retry_count=0):
    ...
    try:
        ...
        # Find the main product page article
        product_article = soup.select_one('article.product_page')
        if not product_article:
            print(f"  - WARNING: Could not find product container on {product_url}.")
            return None, retry_count + 1

        # Title
        name_tag = product_article.select_one('h1')
        ...
        # Brand: use category if available, else first word of title
        breadcrumb = soup.select('ul.breadcrumb li a')
        brand = None
        if len(breadcrumb) >= 3:
            brand = breadcrumb[-1].get_text(strip=True)
        if not brand:
        ...
        # Additional info from table
        info_table = product_article.select_one('table.table.table-striped')
        info_lines = []
        if info_table:
            for row in info_table.select('tr'):
                ...
                if th and td:
                    info_lines.append(f"{th.get_text(strip=True)}: {td.get_text(strip=True)}")
        ...
        # Image
        image_tag = product_article.select_one('div#product_gallery img')
        ...
        # Look for <p class="star-rating ..."> in the detail page
        star_tag = product_article.select_one('p.star-rating')
        star_score = None
        if star_tag:
            # The class name after 'star-rating' is the score in words (e.g., 'Three', 'Five', etc.)
            for cls in star_tag.get('class', []):
                if cls != 'star-rating':
                    # Map to number
                    star_map = {'One': 1, 'Two': 2, 'Three': 3, 'Four': 4, 'Five': 5}
                    star_score = star_map.get(cls, None)
                    break
        ...
        # Replace all '£' with '$' in the markdown content
        ...
        while listing_tasks:
            ...
            try:
                ...
                if not product_list:
                    ...
                    continue
                print(f"Found {len(product_list)} products on {url}.")
                for pod in product_list:
                    link_tag = pod.select_one('h3 > a')
                    if link_tag and link_tag.get('href'):
                        # The href is relative to the listing page, may need normalization
                        product_url = urljoin(url, link_tag['href'])
                        # Check for duplication by product name
                        ...
                        name_tag = temp_soup.select_one('article.product_page h1')
                        product_name = name_tag.get_text(strip=True) if name_tag else "Unknown Product"
                        ...
                        print("  - WARNING: Product pod missing link.")
            except requests.RequestException as e:
    ...
    with open(js_path, 'w', encoding='utf-8') as f:
        ...
        for brand_key in sorted(products_by_brand.keys()):
            ...
            for product in products_by_brand[brand_key]:
                ...
                # Use json.dumps for all string fields to ensure valid JS string literals
                name_escaped = _json.dumps(product['name'], ensure_ascii=False)
                id_escaped = _json.dumps(product['id'], ensure_ascii=False)
                image_escaped = _json.dumps(product['image'], ensure_ascii=False)
                star_value = 'null' if product.get('star') is None else product['star']
                f.write(f"    {{\n")
                f.write(f"      id: {id_escaped},\n")
                f.write(f"      name: {name_escaped},\n")
                f.write(f"      image: {image_escaped},\n")
                f.write(f"      star: {star_value},\n")
