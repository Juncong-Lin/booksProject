  ...
  expandableLinks.forEach((link) => {
    ...
    link.addEventListener("mouseenter", function () {
      ...
      document.querySelectorAll(".expandable.active").forEach((activeLink) => {
        if (
          activeLink !== link &&
          !activeLink.contains(link) &&
          !link.contains(activeLink)
        ) {
      ...
      document.querySelectorAll(".submenu.active").forEach((activeSubmenu) => {
        if (
          ...
          !activeSubmenu.contains(submenu) &&
          !submenu.contains(activeSubmenu)
        ) {
    ...
    link.addEventListener("mouseleave", function (e) {
      setTimeout(() => {
        ...
        if (
          !related ||
          ((!submenu || !submenu.contains(related)) && related !== link)
        ) {
          link.classList.remove("active");
    ...
    link.addEventListener("click", function (e) {
      ...
      // Track sidebar category expansion
      ...
      if (window.innerWidth <= 800) {
        ...
        if (submenu) {
          ...
          currentActiveGroup = submenu.classList.contains("active")
            ? group
            : null;
        }
      }
