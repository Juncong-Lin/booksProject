...
class SubHeaderNavigation {
  ...
  initializeEventListeners() {
    ...
    const subHeaderLinks = document.querySelectorAll(
      ".sub-header-link[data-submenu]"
    );
      ...
      link.addEventListener("click", (event) => {
        ...
        if (isIndexPage && window.loadSpecificCategory) {
          ...
          if (linkText === "Fiction" && window.loadSpecificCategory) {
            ...
            linkText === "Non-Fiction" &&
            window.loadSpecificCategory
          ) {
            window.loadSpecificCategory("Non-Fiction");
            this.setActiveCategory("Non-Fiction");
            window.location.hash = hash;
          } else if (
            linkText === "Children & Young Adult" &&
            window.loadSpecificCategory
          ) {
            ...
            linkText === "Religion & Spirituality" &&
            window.loadSpecificCategory
          ) {
    ...
    document.addEventListener("click", (event) => {
      if (
        !event.target.closest(".sub-header") &&
        !event.target.closest(".sub-header-submenu")
      ) {
        this.hideAllSubmenus();
      }
    });
    ...
    if (allProductsLink) {
      allProductsLink.addEventListener("click", (event) => {
        ...
        const isIndexPage =
          ...
          typeof window.loadAllProducts === "function";
        if (isIndexPage) {
  ...
  expandPrintHeadsMenu() {
    ...
    const printHeadsLink = document.querySelector(
      '[onclick*="loadAllPrintheadProducts"]'
    );
    if (printHeadsLink) {
  ...
  expandInkjetPrintersMenu() {
    ...
    const inkjetLink = document.querySelector(
      '[onclick*="loadSpecificCategory"][onclick*="Inkjet Printers"]:not([onclick*="Eco-Solvent"]):not([onclick*="UV"]):not([onclick*="Solvent"])'
    );
    if (inkjetLink) {
  ...
  expandPrintSparePartsMenu() {
    ...
    const sparePartsLink = document.querySelector(
      '[onclick*="loadAllPrintSpareParts"]'
    );
    if (sparePartsLink) {
  ...
  expandUpgradingKitMenu() {
    ...
    const upgradingKitLink = document.querySelector(
      '[onclick*="loadAllUpgradingKitProducts"]'
    );
    if (upgradingKitLink) {
  ...
  expandMaterialMenu() {
    ...
    const materialLink = document.querySelector(
      '[onclick*="loadAllMaterialProducts"]'
    );
    if (materialLink) {
  ...
  expandLedLcdMenu() {
    ...
    const ledLcdLink = document.querySelector(
      '[onclick*="loadAllLedLcdProducts"]'
    );
    if (ledLcdLink) {
  ...
  expandChannelLetterMenu() {
    ...
    const channelLetterLink = document.querySelector(
      '[onclick*="loadAllChannelLetterProducts"]'
    );
  ...
  handleHashNavigation(hash) {
    ...
    if (hash === "uv-hybrid-konica-km1024i-printers") {
      if (window.loadSpecificCategory) {
        window.loadSpecificCategory(
          "UV Hybrid Inkjet Printer - With Konica KM1024i Printhead"
        );
        this.setActiveCategory("Inkjet Printers");
        this.expandInkjetPrintersMenu();
      }
    ...
    if (hash === "uv-hybrid-ricoh-gen6-printers") {
      if (window.loadSpecificCategory) {
        window.loadSpecificCategory(
          "UV Hybrid Inkjet Printer - With Ricoh Gen6 Printhead"
        );
        this.setActiveCategory("Inkjet Printers");
        this.expandInkjetPrintersMenu();
      }
    ...
    const categoryMap = {
      ...
      "eco-solvent-xp600-printers":
        "Eco-Solvent Inkjet Printers - With XP600 Printhead",
      "eco-solvent-i1600-printers":
        "Eco-Solvent Inkjet Printers - With I1600 Printhead",
      "eco-solvent-i3200-printers":
        "Eco-Solvent Inkjet Printers - With I3200 Printhead",
      "eco-solvent-inkjet-printers---with-xp600-printhead":
        "Eco-Solvent Inkjet Printers - With XP600 Printhead",
      "eco-solvent-inkjet-printers---with-i1600-printhead":
      ...
      "solvent-km512i-printers":
        "Solvent Inkjet Printers - With Konica KM512i Printhead",
      "solvent-km1024i-printers":
        "Solvent Inkjet Printers - With Konica KM1024i Printhead",
      "uv-inkjet-printers": "UV Inkjet Printers",
      "uv-inkjet-printers---with-ricoh-gen6-printhead":
        "UV Inkjet Printers - With Ricoh Gen6 Printhead",
      "uv-konica-km1024i-printers":
        "UV Inkjet Printers - With Konica KM1024i Printhead",
      "sublimation-printers": "Sublimation Printers",
      "sublimation-xp600-printers":
        "Sublimation Printers - With XP600 Printhead",
      "sublimation-i1600-printers":
        "Sublimation Printers - With I1600 Printhead",
      "sublimation-i3200-printers":
    ...
    if (categoryName) {
      ...
      if (
        categoryName === "Inkjet Printers" ||
        categoryName.startsWith("Sublimation Printers")
      ) {
        this.expandInkjetPrintersMenu();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const hasSharedSubheader = document.getElementById(
