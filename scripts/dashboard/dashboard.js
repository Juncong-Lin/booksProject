...
class Dashboard {
  ...
  updateMetrics() {
    ...
    document.getElementById('total-pageviews').textContent = summary.pageViews || 0;
    document.getElementById('unique-sessions').textContent = summary.uniqueSessions || 0;
    document.getElementById('total-clicks').textContent = summary.totalClicks || 0;
    document.getElementById('avg-scroll').textContent = `${summary.avgScrollDepth || 0}%`;

    // Update conversion funnel
    this.updateFunnel(data.historical);
  }

  updateFunnel(events) {
    const pageViews = events.filter(e => e.type === 'page_view').length;
    const productClicks = events.filter(e => e.type === 'click' && e.text?.includes('Add to Cart')).length;
    const cartAdds = events.filter(e => e.type === 'cart_add').length;
    const checkouts = events.filter(e => e.url?.includes('checkout')).length;

    // Update funnel numbers
    document.getElementById('funnel-views').textContent = pageViews;
    document.getElementById('funnel-clicks').textContent = productClicks;
    document.getElementById('funnel-cart').textContent = cartAdds;
    document.getElementById('funnel-checkout').textContent = checkouts;

    // Update funnel bars (relative to page views)
    const maxValue = pageViews || 1;
    document.getElementById('funnel-views-bar').style.width = '100%';
    document.getElementById('funnel-clicks-bar').style.width = `${(productClicks / maxValue) * 100}%`;
    document.getElementById('funnel-cart-bar').style.width = `${(cartAdds / maxValue) * 100}%`;
    document.getElementById('funnel-checkout-bar').style.width = `${(checkouts / maxValue) * 100}%`;
  }
  ...
  createPagePerformanceChart() {
    ...
    const pageMetrics = {};
    events.filter(e => e.type === 'page_end').forEach(event => {
    ...
    const pageData = Object.entries(pageMetrics).map(([url, metrics]) => ({
      page: url.split('/').pop() || 'Home',
      avgTime: metrics.visits > 0 ? Math.round(metrics.totalTime / metrics.visits / 1000) : 0
    })).slice(0, 5); // Top 5 pages

    const ctx = document.getElementById('page-performance-chart').getContext('2d');
    new Chart(ctx, {
      type: 'bar',
      data: {
  ...
  initABTesting() {
    ...
    document.getElementById('ab-test-variant').addEventListener('change', (e) => {
  ...
  trackABTestEvent(variant) {
    if (window.analytics) {
      window.analytics.trackEvent('ab_test_variant_change', { variant: variant });
    }
  }
  ...
  updateABTestResults() {
    ...
    Object.entries(results).forEach(([variant, rate]) => {
      const element = document.getElementById(`${variant.replace('-', '-')}-conversion`);
      if (element) {
